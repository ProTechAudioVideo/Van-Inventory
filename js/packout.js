  ...
  Object.entries(data).forEach(([folderId, folder])=>{
    ...
    const pushRow=(item,index)=>{
      ...
      if(kind==='qty'){
        ...
        if(locked){
          const chip=document.createElement('span'); chip.className=`status-chip ${statusActive||'none'}`; chip.textContent=statusActive?STATUS_LABEL[statusActive]:'—';
          controls.appendChild(chip);
        }else{
          const picker=document.createElement('button');
          picker.className=`status-picker ${statusActive||'none'}`;
          // ▼ CHANGE: show "--" when no status is selected in UNLOCKED mode
          picker.textContent = statusActive ? STATUS_LABEL[statusActive] : '--';
          picker.addEventListener('click',(e)=>{
            e.preventDefault();
            showStatusPopover(picker, statusActive, async (v)=>{
              if(v) items[index].status=v; else delete items[index].status;
